...
| **Fresh thread** | Start with an empty context window.                                 |
| **`handoff`**    | Write a portable markdown file and seed a session anywhere with it. |
| **Subagent**     | Send the task to its own context window and get a report back.      |
| **Compact**      | Use the client's compaction feature to continue from a summary.     |
...
next phase needs this phase as a **primary source**, or the client has enough
effective context capacity for the next phase to fit. Planning → implementation
is the standard yes: the implementation wants the reasoning verbatim, not a
summary of it. Continue costs nothing and loses nothing, so rule it out before
anything else.
...
start a **fresh thread**. It is the cheapest move on the board: it takes no
time and hands back the whole window. The old thread remains available as a
primary source.
...
often. Tell the client what the next phase will be so its summary keeps what
that phase needs.

Compaction is the **default, not the first reach**. It sits at the bottom
because the four questions above it are all cheaper or more precise. The
failure mode when people start here is a fresh session that is confidently
