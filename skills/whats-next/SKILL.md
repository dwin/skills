---
name: whats-next
description: Choose what to do next from the skills and workflows in this plugin.
disable-model-invocation: true
---

# What's Next

Start from the user's situation, then recommend the smallest useful skill or
flow. Prefer the user's established language: plan, docs, review, tickets, and
implementation. Explain the next move without requiring them to remember the
catalog.
...
3. **Branch — is this a multi-session build?**
   - **Yes** → **`to-spec`** (capture the agreed plan as a spec), then **`to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges**. On a local tracker that's one file per ticket under `.scratch/<feature>/issues/`, worked blockers-first by hand; on a real tracker the edges become native blocking links, so any ticket whose blockers are done can be grabbed — kick off **`implement`** per ticket in a fresh thread. Each ticket is self-contained, so the last one's context is disposable.
   - **No** → **`implement`** right here, in the same context window.

   Either way, **`implement`** builds each issue by driving **`tdd`** internally
   — one red-green slice at a time — then closes out by running
   **`code-review`**, a two-axis review (Standards + Spec) of the diff. It leaves
   validated changes in the working tree. When the user wants delivery, run
   **`file-pr`** to commit and file the pull request, then **`babysit-pr`** when
   they want it monitored through CI and review. Reach for **`tdd`** on its own
   to build concrete behaviour test-first, and **`code-review`** on its own to
   review a branch or pull request against a fixed point.

### Context hygiene

Keep steps 1–3 in **one unbroken context window** — don't compact or start a
fresh thread until after `to-tickets` — so the planning, docs, and tickets all
build on the same thinking. Each `implement` then starts fresh, working from
the ticket.

The limit is context pressure: if the client is approaching its effective
reasoning window before `to-tickets`, compact at the nearest phase boundary
and carry on. Do not interrupt a phase merely to create a cleaner boundary.
...
- **Fresh thread** — start with an empty window when nothing here matters to
  what comes next.
...
- **Compact** — use the client's compaction feature to seed the next phase with
  a summary. This is the default at the bottom of the tree, not the first reach.
...
**`setup-dwin-skills`** — run before the first engineering flow to configure
the issue tracker, triage labels, and doc layout the other skills assume.
